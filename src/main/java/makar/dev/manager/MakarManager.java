...
public class MakarManager {
    ...
    // 환승이 없는 경우 막차 시간 구하기
    private Calendar computeLastMakarTime(int dayOfWeek, SubwaySchedule subwaySchedule, int odsayLaneType, int wayCode, int fromStationID, int toStationID) {
    ...
    // 환승이 있는 경우 막차 시간 구하기
    private Calendar computeTransferMakarTime(Calendar takingTime, int dayOfWeek, SubwaySchedule subwaySchedule, int odsayLaneType, int wayCode, int fromStationID, int toStationID) {
        SubwaySchedule.OrdList ordList = getOrdListByDayOfWeek(dayOfWeek, subwaySchedule);
        List<SubwaySchedule.OrdList.TimeDirection.TimeData> time = getTimeByWayCode(wayCode, ordList);
    ...
    @Transactional
    public Calendar findMakarTime(List<SubwaySchedule.OrdList.TimeDirection.TimeData> time, Calendar takingTime, int odsayLaneType, int wayCode, int fromStationID, int toStationID) {
        for (int i = time.size() - 1; i >= 0; i--) {
            ...
            for (int j = timeInfos.size() - 1; j >= 0; j--) {
                ...
                CompletableFuture<Void> task = processLineMap(wayCode, timeInfo, canGoInSubway, result, fromStationID, toStationID, lineMapList);
                tasks.add(task);
            }
    ...
    @Async
    public CompletableFuture<Void> processLineMap(
            int wayCode,
            TimeInfo timeInfo,
            AtomicBoolean canGoInSubway,
            AtomicReference<TimeInfo> result,
            int fromStationID,
            int toStationID,
...
private void waitForTasks(List<CompletableFuture<Void>> tasks) {
    ...
    try {
        ...
        throw new GeneralException(e.getErrorStatus());
